...
def main(address="127.0.0.1", port=3333):
    geetest = Geetest3(address=address, port=port)
    while True:
        captcha = get_captcha()
        if captcha:
            result = geetest.crack(gt=captcha.get('gt'), challenge=captcha.get('challenge'), success=captcha.get('success'))
            status = geetest.status()
            print("=" * 100)
